...
This sequence can be used to measure the spectral-spatial profiles of 
custom shaped RF/gradient pulses. It is based on the standard 'profile' 
sequence originally provided by Agilent, released as open source in
2015. Modified for measuring spectral-spatial pulse profiles, by C. von 
Morze and J. Quirk (WUSTL), Jan 2021. Slice and read gradients have been 
moved to the same axis, for this purpose. To use, simply change the
value of p1pat to the pulse file name (i.e. 'p1pat'.RF and 'p1pat'.GRD).

VNMRJ source code is available via the OpenVNMRJ project on 
github: https://github.com/OpenVNMRJ
...
pulsesequence() {
  ...
  */   
  ...
  msloop(seqcon[1],ns,v3,v4);
    ...
    obspwrf(tpwr2f);
    delay(GDELAY);
    obl_shapedgradient(p1pat,p1,g1,0,0,NOWAIT);   /* JQ changed */
/*    delay(ss_grad.rfDelayFront);*/ /*JQ commented out */
    shapedpulselist(shape90,p1,oph,rof1,rof2,seqcon[1],v4); /*CVM changed for SSRF*/;
...
/*	obl_shapedgradient(ss_grad.name,ssr_grad.duration,-ss_grad.amp,0,0,NOWAIT);*/   /* JQ added */
	obl_shapedgradient(ror_grad.name,ssr_grad.duration,ror_grad.amp,0,0,WAIT);   /* JQ added*/
...
/*    obspower(tpwr2);
    obspwrf(tpwr2f);
    delay(GDELAY);
    obl_shapedgradient(ss2_grad.name,ss2_grad.duration,ss2_grad.amp,0,0,NOWAIT); */;   /* JQ changed */
